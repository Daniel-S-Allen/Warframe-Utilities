...
GlassStyle::GlassStyle() : GlassStyle(styleBase()) {}

GlassStyle::GlassStyle(QStyle *style) : QProxyStyle(style) {}

QStyle *GlassStyle::styleBase(QStyle *style) const {
#ifdef _DEBUG
	for (auto s : QStyleFactory::keys()) {
		OutputDebugString(L"TEST");
		OutputDebugString(reinterpret_cast<LPCWSTR>(s.utf16()));
	}
...
QStyle *GlassStyle::baseStyle() const { return styleBase(); }

void GlassStyle::polish(QPalette &palette) {
...
void GlassStyle::polish(QApplication *app) {
  if (!app) return;
